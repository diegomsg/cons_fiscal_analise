...
inner_join(gestores, unidades, by = join_by(id)) %>%
  select(função, Nome = nome_completo, torre, unidade) %>%
...
Primeira comperência disponível: `r min(contas$Competência) %>% format("%m/%Y")`.
...
::: callout-caution
Outra questão que impacta a inadimplência corrente é a falta de informação no balancete quanto aos acordos realizados, que não mencionam a competência original dos documentos que passaram por negociação, mencionando a informação do acordo como a competência corrente. Considerar essa informação quando observar a inadimplência corrente.
:::
